    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-900 leading-tight">
            @if(isset($progress) && $progress < 100)
                👋 Halo, {{ $user->name }}! Mari Setup Bot Anda.
            @else
                {{ __('Dashboard') }}
            @endif
        </h2>
        ...
        <div class="py-10 px-4 max-w-5xl mx-auto min-h-screen">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8">
                <div class="flex justify-between items-end mb-2">
                    <div>
                        <h3 class="font-bold text-lg text-gray-800">Progress Setup Chatbot</h3>
                        <p class="text-sm text-gray-500">Selesaikan langkah di bawah agar bot siap melayani pelanggan.</p>
                    </div>
                    <span class="font-black text-2xl text-indigo-600">{{ $progress }}%</span>
                </div>
            ...
            <div class="space-y-6">
                <div class="bg-white p-6 rounded-2xl shadow-sm border {{ $step1_done ? 'border-green-200 bg-green-50/30' : 'border-gray-200' }} flex flex-col md:flex-row gap-6 items-start md:items-center transition hover:shadow-md">
                    <div class="flex-shrink-0">
                        @if($step1_done)
                            <div class="w-14 h-14 bg-green-100 text-green-600 rounded-full flex items-center justify-center border-2 border-green-500">
                                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                            </div>
                        @else
                            <div class="w-14 h-14 bg-gray-100 text-gray-400 rounded-full flex items-center justify-center font-black text-2xl border-2 border-gray-300">1</div>
                        @endif
                    </div>
                    <div class="flex-1">
                        <h4 class="font-bold text-xl text-gray-800 mb-1 {{ $step1_done ? 'line-through text-green-700' : '' }}">Ajari Bot Anda (Isi SOP & Pengetahuan)</h4>
                        <p class="text-gray-600 text-sm">Bot AI ini cerdas, tapi dia belum tahu tentang bisnis Anda. Berikan informasi profil perusahaan, jam buka, cara order, dan SOP pelayanan agar bot bisa menjawab seperti CS profesional.</p>
                    </div>
                    <div class="flex-shrink-0 w-full md:w-auto mt-4 md:mt-0">
                        <a href="{{ route('member.pk') }}" class="block w-full text-center px-6 py-3 rounded-xl font-bold transition {{ $step1_done ? 'bg-white border-2 border-green-600 text-green-600 hover:bg-green-50' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md' }}">
                            {{ $step1_done ? 'Edit SOP' : 'Mulai Step 1' }}
                        </a>
                    </div>
                </div>

                <div class="bg-white p-6 rounded-2xl shadow-sm border {{ $step2_done ? 'border-green-200 bg-green-50/30' : 'border-gray-200' }} flex flex-col md:flex-row gap-6 items-start md:items-center transition hover:shadow-md">
                    <div class="flex-shrink-0">
                        @if($step2_done)
                            <div class="w-14 h-14 bg-green-100 text-green-600 rounded-full flex items-center justify-center border-2 border-green-500">
                                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                        ...
                        @else
                            <div class="w-14 h-14 bg-gray-100 text-gray-400 rounded-full flex items-center justify-center font-black text-2xl border-2 border-gray-300">2</div>
                        @endif
                    </div>
                    <div class="flex-1">
                        <h4 class="font-bold text-xl text-gray-800 mb-1 {{ $step2_done ? 'line-through text-green-700' : '' }}">Tambahkan Katalog Produk / Jasa</h4>
                        <p class="text-gray-600 text-sm">Masukkan daftar produk atau jasa yang Anda jual beserta harganya. Bot akan otomatis menawarkan produk ini jika ada pelanggan yang bertanya tentang harga atau rekomendasi barang.</p>
                    </div>
                    <div class="flex-shrink-0 w-full md:w-auto mt-4 md:mt-0">
                        <a href="{{ route('catalogs.index') }}" class="block w-full text-center px-6 py-3 rounded-xl font-bold transition {{ $step2_done ? 'bg-white border-2 border-green-600 text-green-600 hover:bg-green-50' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md' }}">
                            {{ $step2_done ? 'Kelola Katalog' : 'Mulai Step 2' }}
                        </a>
                    </div>
                </div>

                <div class="bg-white p-6 rounded-2xl shadow-sm border {{ $step3_done ? 'border-green-200 bg-green-50/30' : 'border-gray-200' }} flex flex-col md:flex-row gap-6 items-start md:items-center transition hover:shadow-md">
                    <div class="flex-shrink-0">
                        @if($step3_done)
                            <div class="w-14 h-14 bg-green-100 text-green-600 rounded-full flex items-center justify-center border-2 border-green-500">
                                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                        ...
                        @else
                            <div class="w-14 h-14 bg-gray-100 text-gray-400 rounded-full flex items-center justify-center font-black text-2xl border-2 border-gray-300">3</div>
                        @endif
                    </div>
                    <div class="flex-1">
                        <h4 class="font-bold text-xl text-gray-800 mb-1 {{ $step3_done ? 'line-through text-green-700' : '' }}">Hubungkan ke WhatsApp (Scan QR)</h4>
                        <p class="text-gray-600 text-sm">Siapkan HP yang berisi nomor WhatsApp CS Anda (disarankan nomor khusus bisnis). Scan QR Code di sistem kami agar Bot AI bisa mengambil alih dan membalas pesan secara otomatis.</p>
                    </div>
                    <div class="flex-shrink-0 w-full md:w-auto mt-4 md:mt-0">
                        <a href="{{ route('member.whatsapp.setup') }}" class="block w-full text-center px-6 py-3 rounded-xl font-bold transition {{ $step3_done ? 'bg-white border-2 border-green-600 text-green-600 hover:bg-green-50' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md' }}">
                            {{ $step3_done ? 'Lihat Status WA' : 'Mulai Step 3' }}
                        </a>
                    </div>
                </div>

                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col md:flex-row gap-6 items-start md:items-center transition hover:shadow-md {{ $progress == 100 ? 'ring-2 ring-indigo-500' : 'opacity-70' }}">
                    <div class="flex-shrink-0">
                        <div class="w-14 h-14 bg-gray-100 text-gray-400 rounded-full flex items-center justify-center font-black text-2xl border-2 border-gray-300">4</div>
                    </div>
                    <div class="flex-1">
                        <h4 class="font-bold text-xl text-gray-800 mb-1">Testing Chatbot Anda</h4>
                        <p class="text-gray-600 text-sm">Gunakan nomor WhatsApp lain milik Anda, lalu cobalah chat ke nomor bisnis yang sudah terhubung tadi. Pura-puralah menjadi pelanggan yang bertanya tentang produk Anda untuk melihat kecerdasan bot-nya!</p>
                    </div>
                    <div class="flex-shrink-0 w-full md:w-auto mt-4 md:mt-0">
                        @if($progress == 100)
                            <button disabled class="px-6 py-3 rounded-xl font-bold bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed">Siap Dites!</button>
                        @else
                            <button disabled class="px-6 py-3 rounded-xl font-bold bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed">Selesaikan Step 1-3 Dulu</button>
                        @endif
                    </div>
                </div>
            </div>
